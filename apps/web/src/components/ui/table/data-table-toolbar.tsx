...
export function DataTableToolbar<TData>({
  ...
  const columns = React.useMemo(
    () =>
      table
        .getAllColumns()
        .filter((column) => column.getCanFilter() && !column.columnDef.meta?.toolbarHidden)
        .sort(
          (a, b) =>
            (a.columnDef.meta?.toolbarOrder ?? Number.MAX_SAFE_INTEGER) -
            (b.columnDef.meta?.toolbarOrder ?? Number.MAX_SAFE_INTEGER)
        ),
    [table]
  );
  ...
  return (
    ...
    >
      <div className='flex min-w-0 items-center gap-2 overflow-x-auto'>
        ...
        {isFiltered && (
          <Button
            aria-label={commonUiCopy.resetFilters}
            variant='outline'
            size='sm'
