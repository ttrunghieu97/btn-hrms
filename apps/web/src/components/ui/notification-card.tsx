...
import { notificationCardCopy } from '@/locales/vi/system-ui';
...
const formatDate = (date: string | Date): string => {
  ...
  if (diffMins < 1) return notificationCardCopy.justNow;
  if (diffMins < 60) return notificationCardCopy.minutesAgo(diffMins);
  if (diffHours < 24) return notificationCardCopy.hoursAgo(diffHours);
  if (diffDays < 7) return notificationCardCopy.daysAgo(diffDays);
        ...
        <div className='flex items-start justify-between gap-3'>
          ...
          {isUnread && onMarkAsRead && (
            <button
              ...
              aria-label={notificationCardCopy.markAsRead}
            >
              <Icons.check size={16} />
            </button>
          )}
