...
type DisplayConversation = Conversation & { displayName: string };
  ...
  return (
    <div className='border-border/40 bg-background/75 flex flex-col gap-3 rounded-2xl border p-3 backdrop-blur sm:gap-4 sm:rounded-3xl sm:p-4 lg:hidden'>
      <div className='flex items-center justify-between gap-2 sm:gap-3'>
        <div>
          <p className='text-foreground text-xs font-semibold sm:text-sm'>{chatUiCopy.title}</p>
          <p className='text-muted-foreground text-[0.65rem] sm:text-xs'>
            {chatUiCopy.conversationCount(conversations.length)}
          </p>
        </div>
        <Badge
          variant='outline'
