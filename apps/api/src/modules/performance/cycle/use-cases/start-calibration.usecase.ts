...
@Injectable()
export class StartCalibrationUseCase {
  constructor(
  ...
  async execute(id: string, actorUserId: string): Promise<void> {
    ...
    if (cycle.status !== "manager_review") throwBadRequest(
      'Cycle must be in "manager_review" status to transition to "calibration"',
      ERROR_CODES.INVALID_REQUEST,
    );
    await this.repo.update(id, { status: "calibration" });
