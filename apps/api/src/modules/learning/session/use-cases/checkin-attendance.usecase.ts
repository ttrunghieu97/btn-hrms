...
export class CheckInAttendanceUseCase {
  ...
  async execute(sessionId: string, employeeId: string): Promise<void> {
    ...
    if (attendee.status !== "registered") throwBadRequest("Can only check-in registered attendees", ERROR_CODES.INVALID_REQUEST);
    const session = await this.repo.findById(sessionId);
    await this.repo.updateAttendee(attendee.id, { status: "attended", checkedInAt: new Date() });
    await this.eventOutbox.stage(new LearningAttendanceMarkedEvent({ sessionId, courseId: session!.courseId, employeeId, status: "attended" }));
  }
}