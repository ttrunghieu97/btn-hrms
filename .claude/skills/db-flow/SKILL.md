...
## Steps
1. Identify intent:
   - Schema update in dev -> `npm --prefix apps/api run db:push`
   - Generate migration files -> `npm --prefix apps/api run db:generate` then `npm --prefix apps/api run db:migrate`
   - Reset local DB -> `npm --prefix apps/api run db:reset`
   - Seed only -> `npm --prefix apps/api run db:seed`
   - Prep DB for dev startup -> `npm --prefix apps/api run db:prepare`
