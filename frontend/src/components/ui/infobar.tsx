...
import { infoSidebarCopy } from '@/locales/vi/system-ui';
      ...
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        >
          <SheetHeader className='sr-only'>
            <SheetTitle>{infoSidebarCopy.mobileTitle}</SheetTitle>
            <SheetDescription>{infoSidebarCopy.mobileDescription}</SheetDescription>
          </SheetHeader>
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={cn(
          'relative w-(--infobar-width) bg-transparent motion-safe:transition-[width] motion-safe:duration-(--infobar-transition-duration,200ms) motion-safe:ease-linear',
      ...
      <div
        ...
        className={cn(
          'fixed inset-y-0 z-30 hidden h-dvh w-(--infobar-width) motion-safe:transition-[left,right,width] motion-safe:duration-(--infobar-transition-duration,200ms) motion-safe:ease-linear md:flex',
          side === 'left'
...
function InfobarTrigger({ className, onClick, ...props }: React.ComponentProps<typeof Button>) {
  ...
  return (
    <Button
      ...
      aria-label={infoSidebarCopy.toggle}
      onClick={(event) => {
        onClick?.(event);
        toggleInfobar();
      }}
    ...
    >
      ...
      <span className='sr-only'>{infoSidebarCopy.toggle}</span>
    </Button>
  );
}
...
function InfobarRail({ className, ...props }: React.ComponentProps<'button'>) {
  ...
  return (
    <button
      ...
      title={infoSidebarCopy.toggle}
      className={cn(
        'hover:after:bg-sidebar-border absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 motion-safe:transition-all motion-safe:ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] sm:flex',
...
function InfobarGroupLabel({
  ...
  return (
    <Comp
      ...
      className={cn(
        'text-sidebar-foreground/70 ring-sidebar-ring flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium outline-hidden motion-safe:transition-[margin,opacity] motion-safe:duration-200 motion-safe:ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
